...
{
    mica_trigger_irq(g_ped_mica_config->ipc_irq_num);
    return MICA_SUCCESS;
}
...
static void ped_baremetal_receive_message(void)
{
...
static int ped_baremetal_init_irq(struct mica_config *config)
{
    int ret;
...
static int ped_baremetal_init_rpmsg(struct mica_config *config)
{
    void *rsc_table;
...
static const struct mica_pedestal_ops baremetal_ops = {
    ...
    .rcv_message = ped_baremetal_receive_message,
