...
func (f *PedestalFacade) MaxCPUNum() uint32 {
	return f.impl.MaxCPUNum()
}

// MemoryMB returns the amount of free and total memory in MB.
func (f *PedestalFacade) MemoryMB() (free, total uint32) {
	return f.impl.MemoryMB()
}
...
func (f *PedestalFacade) MemHighThreshold() uint32 {
	return f.impl.MemHighThreshold()
}

// HostCPUSeta returns the host CPU set.
func (f *PedestalFacade) HostCPUSeta() cpuset.CPUSet {
	return f.impl.HostCPUSeta()
}
...
func (f *PedestalFacade) SetCPUAffinity(clientID string, cpus cpuset.CPUSet) error {
	if cs := f.getCPUScheduler(); cs != nil {
		return cs.SetCPUAffinity(clientID, cpus)
	}
...
// Returns ErrNotSupported if the pedestal doesn't support CPU scheduling.
func (f *PedestalFacade) SetCPUWeight(clientID string, weight uint32) error {
	if cs := f.getCPUScheduler(); cs != nil {
		return cs.SetCPUWeight(clientID, weight)
	}
	return ErrNotSupported
}
...
// Returns ErrNotSupported if the pedestal doesn't support CPU scheduling.
func (f *PedestalFacade) SetCPUCapacity(clientID string, capacity uint32) error {
	if cs := f.getCPUScheduler(); cs != nil {
		return cs.SetCPUCapacity(clientID, capacity)
	}
	return ErrNotSupported
}
...
func (f *PedestalFacade) Pause(clientID string) error {
	if lm := f.getLifecycleManager(); lm != nil {
		return lm.Pause(clientID)
	}
	return ErrNotSupported
}
...
// Returns ErrNotSupported if the pedestal doesn't support lifecycle management.
func (f *PedestalFacade) Resume(clientID string) error {
	if lm := f.getLifecycleManager(); lm != nil {
		return lm.Resume(clientID)
	}
	return ErrNotSupported
}
...
func (f *PedestalFacade) ClientState(clientID string) (string, error) {
	if sq := f.getStateQuerier(); sq != nil {
		return sq.ClientState(clientID)
	}
	return "", ErrNotSupported
}
...
func (f *PedestalFacade) SetMemory(clientID string, memMB uint32) error {
	if mm := f.getMemoryManager(); mm != nil {
		return mm.SetMemory(clientID, memMB)
	}
	return ErrNotSupported
}
...
// Returns ErrNotSupported if the pedestal doesn't support dynamic memory management.
func (f *PedestalFacade) SetMaxMemory(clientID string, memMB uint32) error {
	if mm := f.getMemoryManager(); mm != nil {
		return mm.SetMaxMemory(clientID, memMB)
	}
	return ErrNotSupported
}
