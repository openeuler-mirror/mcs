...
交付规格快照见[交付规格](../reference/spec.md)。八条验收标准与测试入口一一对应。
...
| 4 | attach 输入 help/uname 得响应 | `test-qemu-features` case 15 + `test-qemu-lifecycle` case 12：断言**真实命令响应**（`UniProton [0-9]` 版本应答或 `command not found`，显式排除输入回显与启动 banner）+ `test-io-qemu` 的 `expect_shell_output`（任一真实命令应答即过：`support shell commond|Available commands:|command not found|UniProton [0-9]`，需专门环境）+ `test-k3s-interaction`（kubectl attach，同一判据族——任一真实命令应答即证明 attach 双向通路，防回显防 banner） | ✅ |
