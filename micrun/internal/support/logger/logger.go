...
const (
	...
	DefaultConfigPath = defs.MicrunLogConfigPath

	// DefaultLogFile is the default log file path for debug builds
	DefaultLogFile = defs.MicrunLogFile

	// DefaultLogConfigPathEnv is the optional env var for override of DefaultConfigPath.
	DefaultLogConfigPathEnv = "MICRUN_LOG_CONFIG"

	// DefaultLogFileEnv is the optional env var for override of DefaultLogFile.
	DefaultLogFileEnv = "MICRUN_LOG_FILE"
...
func LoadConfig(configPath string) (*Config, error) {
	...
	if configPath == "" {
		configPath = resolveDefaultLoggerConfigPath()
	} else {
		configPath = normalizeWhitespace(configPath)
		if configPath == "" {
			configPath = resolveDefaultLoggerConfigPath()
		}
	}
	...
	level := normalizeWhitespace(cfg.Log.Level)
	cfg.Log.Level = level
	if level == "" {
		cfg.Log.Level = "info"
	}
	cfg.Log.File = normalizeWhitespace(cfg.Log.File)
	if cfg.Log.File == "" {
		cfg.Log.File = defaultLoggerLogFile()
	}

	return cfg, nil
}

func resolveDefaultLoggerConfigPath() string {
	if p := normalizeWhitespace(os.Getenv(DefaultLogConfigPathEnv)); p != "" {
		return p
	}
	return DefaultConfigPath
}

func defaultLoggerLogFile() string {
	if p := normalizeWhitespace(os.Getenv(DefaultLogFileEnv)); p != "" {
		return p
	}
...
func swapActiveDebugLogFile(file io.Closer) error {
	debugLogFileMu.Lock()
	defer debugLogFileMu.Unlock()

	if activeDebugFile == nil {
...
func closeActiveDebugLogFile() error {
	debugLogFileMu.Lock()
	defer debugLogFileMu.Unlock()
...
func ensureLogDirectory(filePath string) error {
	...
	resolvedDir, err := resolveSymlinkedPath(dir)
	if err != nil {
		return err
	}
	return os.MkdirAll(resolvedDir, 0755)
}
...
func currentContextFields() contextFields {
	namespaceMutex.RLock()
	ns := currentNamespace
	...
	return contextFields{namespace: ns, containerID: id}
}

func addContextFields(fields logrus.Fields) {
	context := currentContextFields()
	if context.namespace != "" && fields[NamespaceKey] == nil {
		fields[NamespaceKey] = context.namespace
	}
	if context.containerID != "" && fields[IDKey] == nil {
		fields[IDKey] = context.containerID
	}
}
