...
func TestCPUResourceMapping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证 CPU 容量
			if res.CPUCapacity == nil {
				if tt.expectedCap != 0 {
					t.Errorf("CPUCapacity is nil, want %d", tt.expectedCap)
				}
			} else if *res.CPUCapacity != tt.expectedCap {
				t.Errorf("CPUCapacity = %d, want %d", *res.CPUCapacity, tt.expectedCap)
			}
...
func TestResourceMappingPrinciples(t *testing.T) {
	...
	t.Run("CPU容量为0表示不限制", func(t *testing.T) {
		...
		if res.CPUCapacity == nil || *res.CPUCapacity != 0 {
			t.Errorf("CPU容量应该为0表示不限制，got %v", res.CPUCapacity)
		}
	})
	...
	t.Run("只有cpuset没有quota/period", func(t *testing.T) {
		...
		if res.CPUCapacity == nil || *res.CPUCapacity != 200 {
			t.Errorf("CPU容量应该为200%%，got %v", res.CPUCapacity)
		}
	})
