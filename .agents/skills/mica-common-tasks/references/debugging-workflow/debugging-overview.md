...
调试类任务需要先判断复现环境是否必须保留。来自开发板、特定 pedestal、特定硬件资源或特定 SDK 的问题不应默认改用 QEMU；通用生命周期、service ready、TTY 或 Linux/master 用户态行为问题可建议用 QEMU 建立最小复现环境。

需要通过验证收敛问题时，进入 `../testing-workflow/testing-overview.md`。该入口统一负责环境准备、基础 smoke、专项验证选择，以及 QEMU 保留或释放策略。
